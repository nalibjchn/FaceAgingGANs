...
flags.DEFINE_string("alexnet_pretrained_model", "./checkpoints/pre_trained/alexnet/alexnet.model-292000",
                    "Directory name to save the checkpoints")

flags.DEFINE_string("age_pretrained_model", "./checkpoints/pre_trained/age_classifier/age_classifier.model-300000",
                    "Directory name to save the checkpoints")
...
flags.DEFINE_string("checkpoint_dir", './checkpoints/conv5_lsgan_transfer', "Directory name to save the customer checkpoints")

flags.DEFINE_string("source_checkpoint_dir", './checkpoints/', "Directory name to save the checkpoints")

flags.DEFINE_string("sample_dir", './images/', "Directory name to save the sample images")
...
train_generator = ImageDataGenerator(batch_size=FLAGS.batch_size, height=FLAGS.feature_size, width=FLAGS.feature_size,
                                     ...
                                     root_folder= FLAGS.root_folder,mode='train')
...
def my_train():
    ...
    with tf.Graph().as_default():
        ...
        imgs = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.image_size, FLAGS.image_size, 3])
        true_label_features_128 = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, 128, 128, FLAGS.age_groups])
        true_label_features_64 = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, 64, 64, FLAGS.age_groups])
        false_label_features_64 = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, 64, 64, FLAGS.age_groups])
        age_label = tf.compat.v1.placeholder(tf.int32, [FLAGS.batch_size])
        ...
        # Create a saver.
        model.saver = tf.compat.v1.train.Saver(model.save_d_vars + model.save_g_vars, max_to_keep=200)
        model.alexnet_saver = tf.compat.v1.train.Saver(model.alexnet_vars)
        model.age_saver = tf.compat.v1.train.Saver(model.age_vars)
        ...
        tf.train.queue_runner.start_queue_runners(sess)
        ...
        for step in range(FLAGS.max_steps):
            ...
            #print(format_str % (datetime.now(), step, d_loss, g_loss, fea_loss, age_loss)) #Na Li update: no need to print
            logger.info('step:%d,%.3f,%.3f,%.3f,%.3f' % (step, d_loss, g_loss, fea_loss, age_loss))  #Na Li added
