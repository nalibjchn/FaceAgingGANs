...
class FaceAging(object):
    ...
    def train_age_lsgan_transfer(self, source_img_227, source_img_128, imgs, true_label_fea_128, true_label_fea_64,
        ...
        g_source = tf.compat.v1.image.resize_bilinear(images=g_source, size=[227, 227])
        ...
        d_optim = tf.compat.v1.train.AdamOptimizer(self.learning_rate, beta1=0.5).minimize(self.d_loss,
                                                                                 var_list=self.d_vars)
        train_ops = [d_optim] + self._extra_train_ops
        self.d_optim = tf.group(*train_ops)

        g_optim = tf.compat.v1.train.AdamOptimizer(self.learning_rate, beta1=0.5).minimize(g_loss, var_list=self.g_vars)
