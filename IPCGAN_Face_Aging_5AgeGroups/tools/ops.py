...
def conv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        weights = tf.compat.v1.get_variable('weights', shape=[filter_height, filter_width, input_channels / groups, num_filters])
...
def conv1d(input, filter_width, out_channels, in_channels=None, stride=1,
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        kernel = tf.compat.v1.get_variable('weights', [filter_width, in_channels, out_channels],
                                 initializer=tf.random_normal_initializer(stddev=std))
        conv = tf.nn.conv1d(input, kernel, stride=stride, padding='SAME')

        if with_bias:
            biases = tf.compat.v1.get_variable('b', [out_channels], initializer=tf.constant_initializer(0.0))
...
def instance_norm(x, name):
    with tf.compat.v1.variable_scope(name):
        ...
        offset = tf.compat.v1.get_variable('offset', [x.get_shape()[-1]], initializer=tf.constant_initializer(0.0))
