...
class FaceAging(object):
    def __init__(self,
                 ...
                 dataset_name='../DATA/TrainingSet_CACD2000',  # name of the dataset in the folder ./data
                 log = logging
                 ):
...
#               weigts=(0.002, 0.001, 0)  # the weights of adversarial loss and TV loss
              ):
        ...
        sample_files = file_names[0:self.size_batch] #100
        ...
        for i, label in enumerate(sample_files):
            sample_file_new = sample_files[i]
            if(sample_file_new.find('\\')):
                sample_file_new = sample_files[i].replace('\\', '/') #for windows system
            label = int(str(sample_file_new).split('/')[-1].split('_')[0])
            if 0 <= label <= 5:
                label = 0
            elif 6 <= label <= 10:
                label = 1
            ...
            gender = int(str(sample_file_new).split('/')[-1].split('_')[1])
...
#                 print("\tFAILED >_<!")
        ...
        # epoch iteration  start training
        num_batches = len(file_names) // self.size_batch
        for epoch in range(num_epochs):   #epoch number 50
            if enable_shuffle:
                np.random.shuffle(file_names)
            for ind_batch in range(num_batches):
                ...
                for i, label in enumerate(batch_files):
                    batch_new = batch_files[i]
                    if(batch_new.find('\\')):
                        batch_new = batch_files[i].replace('\\','/') #for windows system
                    # label = int(str(batch_files[i]).split('/')[-1].split('_')[0])
                    label = int(str(batch_new).split('/')[-1].split('_')[0])
                    if 0 <= label <= 5:
                        label = 0
                    elif 6 <= label <= 10:
                        label = 1
                    ...
                    gender = int(str(batch_new).split('/')[-1].split('_')[1])
...
#                 print("\tGi=%.4f\tDi=%.4f\tDiG=%.4f" % (Gi_err, Di_err, DiG_err))
                ...
                ##write into logfile loss function tracking logfile
                self.log.info("\nEpoch: [%3d/%3d] Batch: [%3d/%3d]\n\tEG_err=%.4f\tTV=%.4f" %
                    (epoch+1, num_epochs, ind_batch+1, num_batches, EG_err, TV))
                self.log.info("\tEz=%.4f\tDz=%.4f\tDzp=%.4f" % (Ez_err, Dz_err, Dzp_err))
                self.log.info("\tGi=%.4f\tDi=%.4f\tDiG=%.4f" % (Gi_err, Di_err, DiG_err))
#edit by na li end
                ...
                self.log.info("\tTime left: %02d:%02d:%02d" %
                      (int(time_left / 3600), int(time_left % 3600 / 60), time_left % 60))
                self.log.info("\n\tDatetime " + time.ctime())
...
#         query_images = np.tile(images, [1, 1, 1, 1]) #for debug linda
        query_gender = np.tile(gender, [self.num_categories, 1])
#         query_gender = np.tile(gender, [1, 1]) #for debug linda
        z, G = self.session.run(
            [self.z,
             self.G],
            feed_dict={
