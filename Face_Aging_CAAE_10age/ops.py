...
def load_image(
    ...
    if is_gray:
        ...
        image = imread(image_path, pilmode='RGB').astype(np.float32)  #linda
#     image = imresize(image, [image_size, image_size]) #update by Na Li
        image = resize(image, [image_size, image_size]) #Update new function by Na Li
